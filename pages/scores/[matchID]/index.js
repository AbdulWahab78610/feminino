...
export default function Competitions() {
  ...
  const { name = "National Women’s Soccer League", isFavorite } =
    allCompData?.find((value) => value.competition_id === matchID) || {};
  ...
  return (
    <>
      ...
      >
        <Link href="/scores">
          <span className={classes.breadcrumbsMain}>SCORES</span>
        </Link>
        <span className={classes.breadcrumbs}>{initials}</span>
        <span className={classes.breadcrumbs}>Match</span>
      </Breadcrumbs>
      <div className={classes.card}>
        <Image src="/images/t2.svg" alt="comp logo" width="65" height="55" />
        <div className={classes.name}>{name}</div>
        <StarIcon className={isFavorite ? classes.isFavorite : classes.star} />
      </div>
